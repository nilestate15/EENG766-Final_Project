...
def RAIM_chi2(curr_x, curr_P, R, H, K, res_win):
    ...
    Returns:
        nothing
    '''
    # Identity Matrix of state vector size for Resisdual Cov
    I = np.eye(8)
    ...
    # Residual Covariance
    res_cov = (I - K.dot(H)).dot(R).dot(I - (H.dot(K))).T - H.dot(I - K.dot(H)).dot(P0).dot(I - K.dot(H)).T.dot(H.T)

    # Find alpha for noncentral chi square threshold
    inv_R = la.inv(R)
